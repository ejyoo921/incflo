...
namespace 
{
    ...
    AMREX_GPU_HOST_DEVICE AMREX_FORCE_INLINE
    amrex::Real visc_0(const amrex::Real Temp1)
    ...
    AMREX_GPU_HOST_DEVICE AMREX_FORCE_INLINE
    amrex::Real cp_0(const amrex::Real Temp1)
    ...
    AMREX_GPU_HOST_DEVICE AMREX_FORCE_INLINE
    amrex::Real cp_1(const amrex::Real Temp1)
    ...
    AMREX_GPU_HOST_DEVICE AMREX_FORCE_INLINE
    amrex::Real cp_2(const amrex::Real Temp1)
    ...
    AMREX_GPU_HOST_DEVICE AMREX_FORCE_INLINE
    amrex::Real cp_3(const amrex::Real Temp1)
    ...
    AMREX_GPU_HOST_DEVICE AMREX_FORCE_INLINE
    amrex::Real cp_4(const amrex::Real Temp1)
    ...
    AMREX_GPU_HOST_DEVICE AMREX_FORCE_INLINE
    amrex::Real cp_5(const amrex::Real Temp1)
    { //cp calc for slag
        amrex::Real Temp = Temp1 + 273.15;
    ...
    AMREX_GPU_HOST_DEVICE AMREX_FORCE_INLINE
    amrex::Real k_3(const amrex::Real Temp1)
    {   //k calc for 1 % C bath
        amrex::Real Temp = Temp1 + 273.15;
        amrex::Real k3a = 30.0;
    ...
    AMREX_GPU_HOST_DEVICE AMREX_FORCE_INLINE
    amrex::Real k_4(const amrex::Real Temp1) 
    ...
    AMREX_GPU_HOST_DEVICE AMREX_FORCE_INLINE
    amrex::Real k_5(const amrex::Real Temp1)
    {   //k calc for slag  
        amrex::Real Temp = Temp1 + 273.15;
        amrex::Real k_5a = 1.0;
    ...
    AMREX_GPU_HOST_DEVICE AMREX_FORCE_INLINE
    amrex::Real rho_3(const amrex::Real Temp1)
    ...
    AMREX_GPU_HOST_DEVICE AMREX_FORCE_INLINE
    amrex::Real rho_4(const amrex::Real Temp1)
    ...
    AMREX_GPU_HOST_DEVICE AMREX_FORCE_INLINE
    amrex::Real rho_5(const Real Temp1)
    {   //rho calc for slag  
        amrex::Real Temp = Temp1 + 273.15;
    ...
    AMREX_GPU_HOST_DEVICE AMREX_FORCE_INLINE
    amrex::Real liqfrac_0(const amrex::Real Temp1)
    ...
    AMREX_GPU_HOST_DEVICE AMREX_FORCE_INLINE
    amrex::Real liqfrac_1(const amrex::Real Temp1)
    { // a value of 1 indicates liquid, a value of 0 is solid.
        amrex::Real Temp = Temp1 + 273.15;
    ...
    AMREX_GPU_HOST_DEVICE AMREX_FORCE_INLINE
    amrex::Real liqfrac_2(const amrex::Real Temp1)
    {  // a value of 1 indicates liquid, a value of 0 is solid.
        amrex::Real Temp = Temp1 + 273.15;
    ...
    AMREX_GPU_HOST_DEVICE AMREX_FORCE_INLINE
    amrex::Real liqfrac_3(const amrex::Real Temp1)
    ...
    AMREX_GPU_HOST_DEVICE AMREX_FORCE_INLINE
    amrex::Real liqfrac_4(const amrex::Real Temp1)
    ...
    AMREX_GPU_HOST_DEVICE AMREX_FORCE_INLINE
    amrex::Real liqfrac_5(const amrex::Real Temp1)
    ...
    {   // calculate cp based on phase and temp
        ...
        switch (phase) {
            ...
            case 4:
            cpcalc = cp_4(Temp); //cp calc for 3% carbon bath  
            break;
...
void incflo::update_properties ()
{
    ...
    // EY: Temporary test to check if MFIter input matters
        ...
        // for (MFIter mfi(ld.cp_steel, TilingIfNotGPU()); mfi.isValid(); ++mfi)
        // {
        //     // Box const& gbx = mfi.growntilebox(); //bigger box (with ghosts)
        //     Box const& bx = mfi.tilebox();
        ...
        //             // update rho ----------------------------------------------------
        //             amrex::Real cp_fe         = compute_cp(Temp, 0); 
        //             amrex::Real cp_slg        = compute_cp(Temp, 0);

        //             cp_arr(i,j,k,n) = cp_fe;
        //         }
        //     }); //i,j,k
        // } //mfi for cp_steel

        // for (MFIter mfi(ld.k_steel, TilingIfNotGPU()); mfi.isValid(); ++mfi)
        // {
        //     // Box const& gbx = mfi.growntilebox(); //bigger box (with ghosts)
        //     Box const& bx = mfi.tilebox();
        ...
        //             // update rho ----------------------------------------------------
        //             amrex::Real cond_fe         = compute_k(Temp, 0); 
        //             amrex::Real cond_slg        = compute_k(Temp, 0);

        //             cond_arr(i,j,k,n) = cond_fe;
        //         }
        //     }); //i,j,k
        // } //mfi for k_steel
        ...
        for (MFIter mfi(ld.tracer,TilingIfNotGPU()); mfi.isValid(); ++mfi)
        {
            ...
            ParallelFor(bx, [=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept
            {
                ...
                for (int n = 0; n < l_ntrac; ++n)
                {
                    ...
                    if (Temp < 1e-6)
                    {
                        amrex::Abort("Negative temperature!");
                    }

                    amrex::Real vfrac_fe_mix = vfrac_mix_arr(i,j,k,n);
                    amrex::Real vfrac_fe;

                    amrex::Real dens_fe, dens_slg;
                    amrex::Real cp_fe, cp_slg;
                    amrex::Real cond_fe, cond_slg;
                    ...
                    // cond_arr(i,j,k,n) = 0.0365;
                    // cond_arr(i,j,k,n) = 0.000007700;
                    ...
                    {
                        amrex::Print() << "imposing Zero velocity requested" << "\n";
                        if (sol_fe > 0.99)
                        {   // no internal flow
