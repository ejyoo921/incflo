...
void
MyTensorOp::compFlux (int amrlev, const Array<MultiFab*,AMREX_SPACEDIM>& fluxes,
                       MultiFab& sol, Location loc) const
{
#if (AMREX_SPACEDIM == 1)
    amrex::ignore_unused(amrlev, fluxes, sol, loc);
#else
    BL_PROFILE("MyTensorOp::compFlux()");
...
void
MyTensorOp::compVelGrad (int amrlev, const Array<MultiFab*,AMREX_SPACEDIM>& fluxes,
                       MultiFab& sol, Location /*loc*/) const
{
#if (AMREX_SPACEDIM == 1)
    amrex::ignore_unused(amrlev,fluxes,sol);
#else
    BL_PROFILE("MyTensorOp::compVelGrad()");
